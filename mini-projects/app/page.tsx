...
export default function Home() {
  ...
  async function handleSubmit(e: FormEvent<HTMLFormElement>) {
    ...
    const messages = chatLogNew.map((chatMessage) => chatMessage.message).join("/n");

    const repsonse = await fetch("http://localhost:3080", {
      method: "POST",
      headers: {
      ...
      <section className="chatbox">
        <div className="chat-log">
          {chatLog.map((chatMessage, index) => {
            return <ChatMessageRow user={chatMessage.user} message={chatMessage.message} key={index} />
          })}
        </div>
...
const ChatMessageRow = (chatMessage : ChatMessage) => {

  return(
    <div className="chat-message">
      <div className={chatMessage.user === 'user' ? "chat-message-right" : "chat-message-left" }>
        {chatMessage.user === 'chatgpt' ? 
              ...
              : null }
        <div className={"message" + (chatMessage.user === 'user' ? " user" : "")}>
          {chatMessage.message}
        </div>
      </div>
    </div>
